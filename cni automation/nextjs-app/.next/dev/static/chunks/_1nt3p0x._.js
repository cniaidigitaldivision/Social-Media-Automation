...
function AppStoreProvider({ children }) {
    ...
    const toggleWorkspaceSwitcher = (open)=>{
        setState((prev)=>({
                ...prev,
                modals: {
                    ...prev.modals,
                    isWorkspaceSwitcherOpen: open !== undefined ? open : !prev.modals.isWorkspaceSwitcherOpen
                }
            }));
    };
    ...
    const approvePost = (id)=>{
        setState((prev)=>{
            const newItems = prev.approvals.items.filter((i)=>i.id !== id);
    ...
    const retryPipelinePlatform = (id)=>{
        setState((prev)=>{
            if (!prev.postStatusDetail) return prev;
            const newPipeline = prev.postStatusDetail.pipeline.map((p)=>{
                if (p.id === id) {
                    return {
                        ...p,
                        status: 'Publishing...',
    ...
    const refreshToken = (id)=>{
        setState((prev)=>({
                ...prev,
                connectedAccounts: prev.connectedAccounts.map((a)=>a.id === id ? {
                        ...a,
                        isRefreshing: true
                    } : a)
            }));
    ...
    const addLead = (leadData)=>{
        setState((prev)=>{
            const newLead = {
                ...
                initials: (leadData.name || 'NL').split(' ').map((n)=>n[0]).join('').substring(0, 2).toUpperCase(),
    ...
    const openFullPostPreview = (postData)=>{
        setState((prev)=>({
                ...prev,
                modals: {
                    ...prev.modals,
    ...
    const openRequestChangesModal = (id)=>{
        setState((prev)=>({
                ...prev,
                modals: {
                    ...prev.modals,
                    isRequestChangesOpen: true
                }
            }));
    };
    const closeModals = ()=>{
...
_s(AppStoreProvider, "2/eSVXfk2V5ZKttKXeUPXMa6Sd8=");
_c = AppStoreProvider;
...
//# sourceMappingURL=_1nt3p0x._.js.map